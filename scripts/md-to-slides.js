...
<body class="ai-transformation-root">
  ...
  <script>
    ...
    function renderOverview() {
      elOverview.innerHTML = slides.map((s, i) => '\n'
    ...
    function render() {
      ...
      if (s.bullets?.length) html += '<ul class="ai-slide-bullets">' + s.bullets.map((b) => '<li>' + b + '</li>').join('') + '</ul>';
      if (s.buttons?.length) {
